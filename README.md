...
## Current milestone: Phase 4C — Design System Registry, BrandSpec and art direction
...
Phase 4C is deliberately beginning narrowly. The existing Design Contract remains the design authority. The first slice compiles active design choices through a `DesignSystemSpec` intermediate representation before CSS is rendered, with behavioural tests proving every exposed control changes real renderer output. The next step is to persist that compiler output into the ordinary generated repository, then add BrandSpec, ArtDirectionPlan, MotionContract, Component Manifest/Presentation Registry and deterministic DesignLint only alongside real consumers.
