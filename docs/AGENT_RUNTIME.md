...
### The pipeline rehearsal

Every specialist primitive had a unit test and nothing composed them, so "intake → research → … →
release, with independent review at every stage" was an architecture rather than something that had
happened. `npm run rehearse:pipeline` runs it: a registered pipeline from `config/agent-pipelines.json`
with the real role registry, real policy and capability projection, real signed grants, real handoff
and convergence logic and real budget guards. The only stand-in is the specialist —
`tooling/lib/rehearsal-specialist.mjs`, a pure function producing artifact *identities* rather than
content, because content is what a model would contribute and inventing plausible content would make
the evidence read like a build that happened.

What one run establishes, on `marketing-site`: the pipeline is selected by project class and the next
stage is derived from durable artifacts and promoted handoffs rather than from position or a
transcript; all 23 stages run in their **own attempt**, each with its own attempt id, grant fingerprint
and bounded packet — 23 disposable sessions, not one conversation carried through a build; every packet
withholds the artifact kinds its role does not declare it reads and none exceeds its role ceiling;
capability reach is projected from policy and mutation scopes, so a reader role gets strictly fewer
operations and declares no ChangeSet scope; and the stub always claims to be finished, which never
advances anything.

Six of the eight scenarios end in a refusal, which is the point: `human-approval-required` where a
human-reviewed stage has no human; `missing-prerequisite:*` where a declared input does not exist, and
the pipeline does not step over it; `wrong-reviewer:*` for a verdict from a reviewer the pipeline never
registered; refusal at construction for a specialist reviewing its own stage or executing one it does
not own; `undeclared-artifact:*` for an artifact written outside the stage's output scope; and
`no-progress-limit-reached` for a reviewer that keeps returning work to its author. A rework verdict
routes backwards to the role that owns the fix and invalidates every stage after it; a verdict naming a
role no stage owns stops rather than guessing.

**Session loss and resume is not a feature of the rehearsal but its only mode.** Every step re-reads
the ledger and re-derives where the project is, so a second engine over the same state root *is* a
restart. One scenario kills an attempt mid-flight, reconciles the orphan as `lost`, resumes from the
last checkpoint, and finishes byte-identical to an uninterrupted run.

**It closes nothing.** Every promoted stage is still `converged: false` with `gate-not-run`, because
the rehearsal runs no deterministic gate; the durable task closes `blocked` with a named stop reason
and never `succeeded`; no attempt is given a model socket, and the command refuses to start while the
model switch is on. `task-baseline` still has no digest, and the report says so as an operator action
rather than inventing one. This is a rehearsal of the control plane: **not a build, not a runtime
proof, and not product evidence.** The attempt lifecycle against a real isolated process remains
`npm run runtime:canary`; the hosted boundary remains `ops/hetzner/verify-agent-boundary.sh`.

`tooling/pipeline-rehearsal.test.mjs` asserts every refusal above, validates each verdict, handoff,
checkpoint and convergence report against its registered schema, and fails if the scenario set ever
