...
Next: **bind evidence to one artifact**. The lifecycle knows what a revision is and the service now records which authority produced it, but nothing yet joins them — source, lock, bundle, preview, browser evidence, review and candidate all still hang off a project rather than off an immutable ArtifactRevision. Until they do, stale evidence can still describe a build that no longer exists. Trace one real human build by digest, show that changing source, lock or bundle invalidates what was measured about the old one, and that a rework creates a child rather than editing its parent.
