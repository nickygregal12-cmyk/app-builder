...
Independent visual review reached best mean 6.70 on thin truth and 6.64 on the rich Ardwell & Roe benchmark, with distinctiveness in the mid-5s to low 6s. **Those numbers are OLD SCALE and are not convertible to the current rubric** — they were scored on nine criteria with no defined meaning for any point on the scale, by a prompt that disclosed the 8.5 bar to the reviewer. #257 replaced that with thirteen anchored criteria and withholds the bar, and no factory build has yet been re-scored under it. The formal gate remains **mean ≥ 8.5 and every criterion ≥ 6.5**. Therefore `0 edits` does not mean `excellent design`; optimise for resources/interventions required to reach independently accepted quality.
...
Reproducible build identity: done. Verification resolves the generated project's lockfile once, installs *from* it with `npm ci`, refuses if the lock moved during installation, and records the source digest, the lock digest, the toolchain it ran under and a digest of the built output. The toolchain is one exact pair in `config/toolchain.json`, read from `.nvmrc` by every workflow; a host standing somewhere else records honestly and cannot claim `buildable`. Generated handover and the Console say `npm ci`. A verified project with a full identity under the declared pair now reaches the ladder's third rung.

The authorization contract exists. `ActionAuthorization` generalises the approved-build-plan semantics to any operation — exact base, project, operation, file scope, environment, risk, budget, expiry, idempotency, single use, no self-approval, scope narrowable and never widenable — and "once" is a unique constraint rather than a read. Authority: `docs/FACTORY_CONTROL_PLANE.md`.

Route parity: done. Every mutating operation is enumerated in `config/mutating-operations.json` and decided in one place inside the service, which is where all three entry points — HTTP, the broker socket, an in-process call — already meet. Once a contract is approved, `project.generate` is refused on every one of them and permitted only through the approved plan, proved against the real server, the real broker dispatch map and a direct call, with every refusal recorded and attributed to the door that asked. An operation with no registry entry is refused, so the next mutating route either registers itself or fails.

Evidence binding: done. A governed build now opens an ArtifactRevision when its contract is approved and climbs it — `materialized` at generation, `buildable` at verification — recording source, lock, toolchain and output digests as it goes. Evidence names which of those it was measured against, and the refusals cover unbound evidence, another revision's evidence, an artifact substituted under its own evidence, and evidence claiming to have measured something not yet recorded. Revisions are a projection of the ledger, so a rebuild from the raw event file reproduces them identically; a real governed build is traced by digest in `tooling/artifact-evidence.test.mjs`.

Lanes: done, and the finding behind them is the useful part. Three deterministic checks — browser E2E, serious/critical axe, executed RLS acceptance — were unregistered, and not for want of a producer. `npm run gates:evidence` builds the NBM project and measures it; the accessibility and browser lanes measure the generated acceptance application. Two builds, two composition hashes, so a registered check resolved `evidence-for-another-build` every run — and because integrity treats any registered check resolving `not-run` as broken wiring, registering one took the `verify` job with it. A check now belongs to the lane that produces its artifact, integrity asks only about the lane that ran, and the rest are named as deferred rather than skipped.

Transport seam: done for the lifecycle families. The Console was carrying a hand-written copy of the project summary while the service built it, and the two fields that had just been added — the lifecycle claim and the build identity — were typed twice with nothing comparing them. The schema is now the family, the Console imports the generated type, and the service asserts the contract on the way out. One family, not a repository-wide migration: a family earns generation by drifting, or by being about to.

Next: **make the evidence converge on one artifact**. Lanes make today honest; they do not make a release candidate possible, because that needs every required gate satisfied by evidence about *one* artifact and the lanes each build their own. Point the accessibility and browser journeys at the artifact the deterministic producers already built, register the three checks against it, and have each record the artifact revision it measured. That is what `behavior-verified` is waiting on, and it is the last rung before the ladder needs a person.
