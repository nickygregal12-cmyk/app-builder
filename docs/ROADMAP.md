...
Next: **one authorization decision for every mutating route**. The approved-build-plan path fingerprints and freezes its inputs and enforces single use; direct generation through the Console, HTTP, MCP and internal callers does not, and a guarantee one route enforces is not a guarantee. Generalise the approved-plan semantics into a bounded ActionAuthorization — exact base, project, operation, scope, environment, risk, budget, expiry, idempotency — reusing the existing grant, approval and risk primitives, then route every equivalent mutating operation through it. Acceptance is route parity under adversarial test: correct authorization succeeds, and Console, HTTP, MCP, agent and internal callers each fail without one.
