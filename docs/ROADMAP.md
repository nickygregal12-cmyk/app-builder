...
Next, and the thing that makes it mean anything: **route parity**. The contract guards nothing until every equivalent mutating route asks it the same question, and today `project.generate` is still reachable without one through the HTTP service, the MCP adapter, the agent broker and internal callers — exactly as `ApprovedBuildPlan` was. Enumerate every mutating operation and every caller, route them through `authorizeAction`, and prove it adversarially: correct authorization succeeds, and Console, HTTP, MCP, agent and internal callers each fail without one, with the decision recorded either way. Preserve the existing FactoryService implementations; this is a front door, not a rewrite.
