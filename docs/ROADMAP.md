...
Next: **behaviour evidence about the exact artifact**. Three deterministic checks — browser E2E, serious/critical axe, executed RLS acceptance — are named by their gates and answered by no producer, so they stay `not-run` while the outputs they would read already exist. Register them, and record the artifact revision each was measured against so they arrive bound rather than needing migrating later. That is what `behavior-verified` is waiting on, and it is the last rung before the ladder needs a person.
