...
**One row is outstanding, and it is not a capability.** Cross-browser visual smoke, the cross-build
anti-template diagnostic and the bespoke-presentation lane are closed. What remains is the independent
visual verdict — and it is the same gate three times over, because a bespoke presentation, a candidate
set and a static rendering all need a reviewer who did not produce them, and no genuinely independent
model runtime is enabled in this repository.

That is a person or a provider, not a piece of engineering, so it does not become closable by building
anything. The machinery it is waiting on is complete: the packet, the scored bar, the bounded rework
loop, the reject-the-whole-set outcome, and now a lane that can build the thing a failed review asks for
when no registry component will do.
