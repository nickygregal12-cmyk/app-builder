...
const PAGES = process.env.PAGES ? process.env.PAGES.split(',') : ['/', '/schedule', '/proving', '/model', '/pilot'];
...
for (const vp of VIEWPORTS) {
  ...
  for (const path of PAGES) {
    ...
    await sharp({ create: { width: vp.width, height: pageHeight, channels: 3, background: '#f2efe9' } })
      .composite(tiles)
      .png()
