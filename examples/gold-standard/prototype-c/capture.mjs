...
const PAGES = process.env.PAGES ? process.env.PAGES.split(',') : ['/', '/colours', '/colours/oxblood', '/colours/straw', '/paints', '/walls', '/lime', '/samples'];
...
for (const vp of VIEWPORTS) {
  ...
  for (const path of PAGES) {
    ...
    await sharp({ create: { width: vp.width, height: pageHeight, channels: 3, background: '#ede8da' } })
      .composite(tiles)
      .png()
