...
for (const rel of required) {
  if (!fs.existsSync(path.join(root, rel))) { console.error(`Missing required foundation file: ${rel}`); failed = true; }
}
for (const rel of ['config/modules.json','config/project-types.json','config/agent-routing.json','questionnaires/v1/base.json','schemas/project-manifest.schema.json','schemas/build-contract.schema.json','schemas/intake-session.schema.json']) {
...
try {
  ...
  for (const [projectType, config] of Object.entries(projectTypes)) {
    ...
    if (!fs.existsSync(questionnaire)) { console.error(`Project type ${projectType} references missing questionnaire: ${config.questionnaire}`); failed = true; }
  }
} catch { failed = true; }
